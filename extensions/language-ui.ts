  ...
  pi.registerCommand("shop-language", {
    ...
    handler: async (args, ctx) => {
      try {
        ...
        if (ctx.sessionManager.getSessionId() !== session) throw new Error(t("Session changed; language not saved"));
        const result = await pi.exec("python3", [join(shopPaths().core_root, "core/language.py"), choice,
        ...
        // Python writes only language.json under its own CAS lock.
