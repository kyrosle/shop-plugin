...
2. Call shop_spawn_worker({id, task, profile}) per task. Use profile "fast" (the default) for routine work; use "steady" only for complex, risky or critical tasks, because it costs more. Spawn independent tasks before waiting so they run in parallel; spawn dependent tasks only after their inputs are accepted.
