...
- 当前适配器要求 Herdr **>= 0.9.0**（实测 **0.9.1**）、协议 **22**、schema **1**。CLI 与正在运行的 server 必须兼容。更新版本只有在协议/schema 一致、且仍声明适配器使用的全部响应类型和字段时才会被接受，否则 Shop 拒绝启动。
...
活动 run、未知写入者、仍在进行的初始化/成员变更会阻止升级。不能用 `idle` 或关闭 pane 推断写入已停止。保留的失败登记本身不代表操作仍在执行，也不是重置其他 tab 的授权；核实该工位执行/变更已停止、没有未知写入者后，用户可明确选择只升级代码、原样保留登记，再到目标 tab 单独恢复。已有/旧工位按[迁移与回退](MIGRATION.md)处理；不要同时启用新旧角色注入扩展。
