...
- 设置采用带标题边框的面板，包含作用域页签和底部快捷键。↑↓ 选字段，Enter 打开模型或思考档位选择。
- 模型选择支持输入搜索，按模型 ID/名称/Provider 模糊匹配；列表固定最多 10 行，矮窗口自动减少。↑↓、PageUp/PageDown 及全屏模式鼠标滚轮滚动，底部显示选中模型名称。Enter 选择，Esc/Ctrl+C 不选择并返回。
- 清空模型搜索可选“继承父层”。选择只修改 Shop 草稿，不切换 Architect 模型、不保存 Pi 默认模型、不刷新 Provider 或调用模型；保存确认仍是独立步骤。
...
- **快速 Worker（低成本）**（`worker`）：日常小任务、批量修改、快速执行。
- **稳健 Worker（重可靠性）**（`worker-2`）：复杂实现、疑难修复、关键改动。

这是同一种 Worker 的两套配置用途，不是两种实现，也不保证所选模型的价格或质量；具体模型由你选择。面板和选择器只调整展示名称，内部 ID、已有配置、扩容及派单规则不变。选中 Worker 字段时显示对应用途说明。
