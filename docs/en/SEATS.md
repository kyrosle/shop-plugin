# Ephemeral seats (preview)

[中文](../zh-CN/SEATS.md) · [README](../../README.md)

An alternative to the resident `/shop` workstation. No panes are opened in advance and there are no ticket files to write. Your Pi (Architect) writes a short SPEC and PLAN; each downstream seat then starts in a new Herdr pane from a child session built from its parent's context, reports once through a tool, and closes its own pane.
...
The same installation, bridge and configuration as Shop (see README). Lead and Worker models and thinking levels come from `/shop-config`; Architect keeps your current Pi model. You do **not** need to open the resident Shop (Ctrl+B → U) first, but the Pi must run inside Herdr.
...
Seat tools: the Lead has `shop_spawn_worker`, `shop_wait_workers` (waits without model calls and reports Worker panes that closed without a report as `lost`) and `shop_report`; Workers have `shop_report`. Seats load only these Shop tools.
...
| `seats/<id>.json` | Pane, model, handoff decision (form, reason, token counts, analyzer usage) |
...
- Preview. Tested on macOS with the isolated live host lane only (see [testing](TESTING.md)).
- Workers share one checkout: parallel tasks must not touch the same files. There is no worktree isolation, commit, integration or shutdown gating; those remain features of the resident `/shop`.
